...
if (! function_exists('nv_array_sciencecat_admin')) {
    function nv_array_sciencecat_admin()
    {
        global $db, $module_data;
        $array_sciencecat_admin = array();
        $sql = 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_admins ORDER BY userid ASC';
        $result = $db->query($sql);
		
        while ($row = $result->fetch()) {
            $array_sciencecat_admin[$row['userid']][$row['catid']] = $row;
        }
        return $array_sciencecat_admin;
    }
}

$is_refresh = false;
$array_sciencecat_admin = nv_array_sciencecat_admin();

$module_admin = explode(',', $module_info['admins']);
// Xoa cac dieu hanh vien khong co quyen tai module
foreach ($array_sciencecat_admin as $userid_i => $value) {
...
foreach ($module_admin as $userid_i) {
    ...
    if ($userid_i > 0 && ! isset($array_sciencecat_admin[$userid_i])) {
...
if (defined('NV_IS_ADMIN_FULL_MODULE')) {
    ...
    if ($nv_Request->isset_request("submit", "post") and $userid > 0) {
        ...
        if ($admin_module == 1 or $admin_module == 2) {
            ...
            $sql = "SELECT catid, title, subcatid FROM " . NV_PREFIXLANG . "_" . $module_data . "_sciencecat ORDER BY sort ASC";
            $result_sciencecat = $db->query($sql);
            while ($row = $result_sciencecat->fetch()) {
    ...
    if (! empty($module_info['admins'])) {
        ...
        while ($row = $result->fetch()) {
            ...
            $admin_module = (isset($array_sciencecat_admin[$userid_i][0])) ? intval($array_sciencecat_admin[$userid_i][0]['admin']) : 0;
            $admin_module_sciencecat = $array_permissions_mod[$admin_module];
            $is_edit = true;
            ...
            $users_list[$row['userid']] = array(
                ...
                'admin_module_sciencecat' => $admin_module_sciencecat,
                'is_edit' => $is_edit );
        }
    }
    ...
    if (! empty($users_list)) {
        ...
        if ($userid > 0 and $userid != $admin_id) {
            $admin_module = (isset($array_sciencecat_admin[$userid][0])) ? intval($array_sciencecat_admin[$userid][0]['admin']) : 0;
            $is_edit = true;
            if ($admin_module == 2 and ! defined('NV_IS_SPADMIN')) {
                $is_edit = false;
            }
            ...
            if ($is_edit) {
                ...
                $sql = "SELECT catid, title, lev FROM " . NV_PREFIXLANG . "_" . $module_data . "_sciencecat ORDER BY sort ASC";
                if ($db->query($sql)->fetchColumn() == 0) {
                    Header("Location: " . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=sciencecat");
                    die();
                }
                ...
                while ($row = $result_sciencecat->fetch()) {
                    ...
                    $u['checked_admin'] = (isset($array_sciencecat_admin[$userid][$row['catid']]) and $array_sciencecat_admin[$userid][$row['catid']]['admin'] == 1) ? " checked=\"checked\"" : "";
                    $u['checked_add_content'] = (isset($array_sciencecat_admin[$userid][$row['catid']]) and $array_sciencecat_admin[$userid][$row['catid']]['add_content'] == 1) ? " checked=\"checked\"" : "";
                    $u['checked_pub_content'] = (isset($array_sciencecat_admin[$userid][$row['catid']]) and $array_sciencecat_admin[$userid][$row['catid']]['pub_content'] == 1) ? " checked=\"checked\"" : "";
                    $u['checked_edit_content'] = (isset($array_sciencecat_admin[$userid][$row['catid']]) and $array_sciencecat_admin[$userid][$row['catid']]['edit_content'] == 1) ? " checked=\"checked\"" : "";
                    $u['checked_del_content'] = (isset($array_sciencecat_admin[$userid][$row['catid']]) and $array_sciencecat_admin[$userid][$row['catid']]['del_content'] == 1) ? " checked=\"checked\"" : "";
                    $u['checked_app_content'] = (isset($array_sciencecat_admin[$userid][$row['catid']]) and $array_sciencecat_admin[$userid][$row['catid']]['app_content'] == 1) ? " checked=\"checked\"" : "";
    ...
    $xtpl = new XTemplate("admin.tpl", NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/" . $module_file);
    $xtpl->assign('LANG', $lang_module);
    $xtpl->assign('CAPTION_EDIT', $lang_module['admin_sciencecat_for_user']);

    $result_sciencecat = $db->query($sql);
    while ($row = $result_sciencecat->fetch()) {
        if (isset($array_sciencecat_admin[$admin_id][$row['catid']])) {
            $u = array();
            $check_show = false;
            if ($array_sciencecat_admin[$admin_id][$row['catid']]['admin'] == 1) {
                $check_show = true;
            } else {
                if ($array_sciencecat_admin[$admin_id][$row['catid']]['add_content'] == 1) {
                    $check_show = true;
                } elseif ($array_sciencecat_admin[$admin_id][$row['catid']]['pub_content'] == 1) {
                    $check_show = true;
                } elseif ($array_sciencecat_admin[$admin_id][$row['catid']]['edit_content'] == 1) {
                    $check_show = true;
                } elseif ($array_sciencecat_admin[$admin_id][$row['catid']]['app_content'] == 1) {
                    $check_show = true;
                }
            }
            if ($check_show) {
                ...
                $u['catid'] = $row['catid'];
                $u['title'] = $xtitle_i . $row['title'];
                $u['checked_admin'] = (isset($array_sciencecat_admin[$admin_id][$row['catid']]) and $array_sciencecat_admin[$admin_id][$row['catid']]['admin'] == 1) ? "X" : "";
                $u['checked_add_content'] = (isset($array_sciencecat_admin[$admin_id][$row['catid']]) and $array_sciencecat_admin[$admin_id][$row['catid']]['add_content'] == 1) ? "X" : "";
                $u['checked_pub_content'] = (isset($array_sciencecat_admin[$admin_id][$row['catid']]) and $array_sciencecat_admin[$admin_id][$row['catid']]['pub_content'] == 1) ? "X" : "";
                $u['checked_edit_content'] = (isset($array_sciencecat_admin[$admin_id][$row['catid']]) and $array_sciencecat_admin[$admin_id][$row['catid']]['edit_content'] == 1) ? "X" : "";
                $u['checked_del_content'] = (isset($array_sciencecat_admin[$admin_id][$row['catid']]) and $array_sciencecat_admin[$admin_id][$row['catid']]['del_content'] == 1) ? "X" : "";
                $u['checked_app_content'] = (isset($array_sciencecat_admin[$admin_id][$row['catid']]) and $array_sciencecat_admin[$admin_id][$row['catid']]['app_content'] == 1) ? "X" : "";
                $xtpl->assign('CONTENT', $u);
