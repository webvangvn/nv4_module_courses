...
foreach ($array_viewsciencecat_full as $key => $val) {
...
if (defined('NV_IS_ADMIN_FULL_MODULE') or ! in_array('admins', $allow_func)) {
    ...
    if (! empty($savepost)) {
        ...
        $nv_Cache->delMod('settings');
        $nv_Cache->delMod($module_name);
        Header('Losciencecation: ' . NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&rand=' . nv_genpass());
        die();
    }
