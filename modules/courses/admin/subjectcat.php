...
$page_title = $lang_module['subjectcat'];
...
if ($catid > 0 and isset($global_array_subjectcat[$catid])) {
    $parentid = $global_array_subjectcat[$catid]['parentid'];
    $title = $global_array_subjectcat[$catid]['title'];
    $titlesite = $global_array_subjectcat[$catid]['titlesite'];
    $alias = $global_array_subjectcat[$catid]['alias'];
    $description = $global_array_subjectcat[$catid]['description'];
    $descriptionhtml = $global_array_subjectcat[$catid]['descriptionhtml'];
    $viewdescription = $global_array_subjectcat[$catid]['viewdescription'];
    $image = $global_array_subjectcat[$catid]['image'];
    $keywords = $global_array_subjectcat[$catid]['keywords'];
    $groups_view = $global_array_subjectcat[$catid]['groups_view'];
    $featured = $global_array_subjectcat[$catid]['featured'];

    if (! defined('NV_IS_ADMIN_MODULE')) {
        if (!(isset($array_subjectcat_admin[$admin_id][$parentid]) and $array_subjectcat_admin[$admin_id][$parentid]['admin'] == 1)) {
    ...
    $caption = $lang_module['edit_subjectcat'];
    $array_in_subjectcat = GetCatidInParent2($catid);
} else {
    $caption = $lang_module['add_subjectcat'];
...
if (! empty($savecat)) {
    ...
    if (empty($_alias) or !preg_match("/^([a-zA-Z0-9\_\-]+)$/", $_alias)) {
        if (empty($alias)) {
            if ($catid) {
                ...
                $_m_subjectcatid = $db->query('SELECT MAX(catid) AS cid FROM ' . NV_PREFIXLANG . '_' . $module_data . '_subjectcat')->fetchColumn();

                if (empty($_m_subjectcatid)) {
                    $alias = 'cat-1';
                } else {
                    $alias = 'cat-' . (intval($_m_subjectcatid) + 1);
                }
            }
        }
    } else {
    ...
    if ($catid == 0 and $title != '') {
        $weight = $db->query('SELECT max(weight) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_subjectcat WHERE parentid=' . $parentid)->fetchColumn();
        $weight = intval($weight) + 1;
        $viewcat = 'viewcat_page_new';
        $subcatid = '';

        $sql = "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . "_subjectcat (parentid, title, titlesite, alias, description, descriptionhtml, image, viewdescription, weight, sort, lev, viewcat, numsubcat, subcatid, inhome, numlinks, newday,featured, keywords, admins, add_time, edit_time, groups_view) VALUES
        ...
        if ($newcatid > 0) {
            ...
            nv_insert_logs(NV_LANG_DATA, $module_name, $lang_module['add_subjectcat'], $title, $admin_info['userid']);
            Header('Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&parentid=' . $parentid);
            die();
        } else {
        ...
        if ($stmt->rowCount()) {
            if ($parentid != $parentid_old) {
                ...
                $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_subjectcat SET weight=' . $weight . ' WHERE catid=' . intval($catid);
                $db->query($sql);

                nv_fix_subjectcat_order();
                nv_insert_logs(NV_LANG_DATA, $module_name, $lang_module['edit_subjectcat'], $title, $admin_info['userid']);
            }
...
$array_subjectcat_list = array();
if (defined('NV_IS_ADMIN_MODULE')) {
    $array_subjectcat_list[0] = $lang_module['cat_sub_sl'];
}
...
if (!empty($array_subjectcat_list)) {
    ...
    while (list($catid_i, $title_i) = each($array_subjectcat_list)) {
        if (!in_array($catid_i, $array_in_subjectcat)) {
            $cat_listsub[] = array(
                'value' => $catid_i,
...
$xtpl = new XTemplate('subjectcat.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
...
$xtpl->assign('CAT_LIST', nv_show_subjectcat_list($parentid));
...
if ($catid > 0) {
    $sql = 'SELECT id FROM ' . NV_PREFIXLANG . '_' . $module_data . '_subject WHERE status=1 ORDER BY publtime DESC LIMIT 100';
    $result = $db->query($sql);
    $array_id=array();
    $array_id[] = $featured;
    ...
    $sql1 = 'SELECT id, title FROM ' . NV_PREFIXLANG . '_' . $module_data . '_subject  WHERE id IN ('.implode(',', $array_id).') ORDER BY publtime DESC';
