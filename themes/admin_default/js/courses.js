 ...
 */
 ...
 function nv_chang_sciencecat(sciencecatid, mod) {
	var nv_timer = nv_settimeout_disable('id_' + mod + '_' + sciencecatid, 5000);
	var new_vid = $('#id_' + mod + '_' + sciencecatid).val();
	$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=change_sciencecat&nocache=' + new Date().getTime(), 'catid=' + sciencecatid + '&mod=' + mod + '&new_vid=' + new_vid, function(res) {
		...
		nv_show_list_sciencecat(parentid);
		return;
	});
	return;
}

function nv_show_list_sciencecat(parentid) {
	if (document.getElementById('module_show_list')) {
		$('#module_show_list').load(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=list_sciencecat&parentid=' + parentid + '&nocache=' + new Date().getTime());
	}
	return;
}

function nv_del_sciencecat(sciencecatid) {
	$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=del_sciencecat&nocache=' + new Date().getTime(), 'catid=' + sciencecatid, function(res) {
		nv_del_sciencecat_result(res);
	});
	return false;
}

function nv_del_sciencecat_result(res) {
	var r_split = res.split('_');
	if (r_split[0] == 'OK') {
		var parentid = parseInt(r_split[1]);
		nv_show_list_sciencecat(parentid);
		...
		if (confirm(nv_is_del_confirm[0])) {
			var sciencecatid = r_split[1];
			var delallcheckss = r_split[2];
			$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=del_sciencecat&nocache=' + new Date().getTime(), 'catid=' + sciencecatid + '&delallcheckss=' + delallcheckss, function(res) {
				nv_del_sciencecat_result(res);
			});
		}
	} else if (r_split[0] == 'ERR' && r_split[1] == 'sciencecat') {
		alert(r_split[2]);
	} else if (r_split[0] == 'ERR' && r_split[1] == 'ROWS') {
		if (confirm(r_split[4])) {
			var sciencecatid = r_split[2];
			var delallcheckss = r_split[3];
			$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=del_sciencecat&nocache=' + new Date().getTime(), 'catid=' + sciencecatid + '&delallcheckss=' + delallcheckss, function(res) {
				$("#edit").html(res);
			});
			parent.losciencecation = '#edit';
		}
	} else {
...
function nv_del_block_sciencecat(bid) {
	if (confirm(nv_is_del_confirm[0])) {
		$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=del_block_sciencecat&nocache=' + new Date().getTime(), 'bid=' + bid, function(res) {
			var r_split = res.split('_');
			if (r_split[0] == 'OK') {
				nv_show_list_block_sciencecat();
			} else if (r_split[0] == 'ERR') {
				alert(r_split[1]);
			} else {
				alert(nv_is_del_confirm[2]);
...
function nv_chang_block_sciencecat(bid, mod) {
	var nv_timer = nv_settimeout_disable('id_' + mod + '_' + bid, 5000);
	var new_vid = $('#id_' + mod + '_' + bid).val();
	$.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=chang_block_sciencecat&nocache=' + new Date().getTime(), 'bid=' + bid + '&mod=' + mod + '&new_vid=' + new_vid, function(res) {
		var r_split = res.split('_');
		if (r_split[0] != 'OK') {
			alert(nv_is_change_act_confirm[2]);
		}
...
function nv_show_list_block_sciencecat() {
	if (document.getElementById('module_show_list')) {
		$('#module_show_list').load(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=list_block_sciencecat&nocache=' + new Date().getTime());
	}
	return;
}
...
function nv_main_action(oForm, checkss, msgnocheck) {
	...
	if (listid != '') {
		...
		if (action == 'delete') {
			...
			window.losciencecation.href = script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '='+action+'&listid=' + listid + '&checkss=' + checkss;
		}
	} else {
		alert(msgnocheck);
	}
...
function nv_check_movesciencecat(oForm, msgnocheck) {
	var fa = oForm['sciencecatidnews'];
	if (fa.value == 0) {
		alert(msgnocheck);
		return false;
	}
...
function nv_del_content_result(res) {
	var r_split = res.split('_');
	if (r_split[0] == 'OK') {
		window.losciencecation.href = window.losciencecation.href;
	} else if (r_split[0] == 'ERR') {
		alert(r_split[1]);
	} else {
		alert(nv_is_del_confirm[2]);
...
$(document).ready(function(){
		...
		if (confirm(LANG.topic_delete_confirm)) {
			$.ajax({
				...
				success : function(data) {
					alert(data);
					window.losciencecation = 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=topicsnews&topicid=' + CFG.topicid;
				}
			});
		}
		return false;
	...
	$("#select-img-topic").click(function() {
		...
		var type = "image";
		nv_open_browse(script_name + "?" + nv_name_variable + "=upload&popup=1&area=" + area + "&path=" + path + "&type=" + type + "&currentpath=" + currentpath, "NVImg", 850, 420, "resizable=no,scrollbars=no,toolbar=no,losciencecation=no,status=no");
		return false;
	});
	...
	$("#select-img-source").click(function() {
		...
		var path = CFG.upload_path;
		var type = "image";
		nv_open_browse(script_name + "?" + nv_name_variable + "=upload&popup=1&area=" + area + "&path=" + path + "&type=" + type, "NVImg", 850, 420, "resizable=no,scrollbars=no,toolbar=no,losciencecation=no,status=no");
		return false;
	});
	...
	$("#select-img-post").click(function() {
		...
		var currentpath = CFG.upload_current;
		var type = "image";
		nv_open_browse(script_name + "?" + nv_name_variable + "=upload&popup=1&area=" + area + "&alt=" + alt + "&path=" + path + "&type=" + type + "&currentpath=" + currentpath, "NVImg", 850, 420, "resizable=no,scrollbars=no,toolbar=no,losciencecation=no,status=no");
		return false;
	});
	...
	$('#update-topic').click(function() {
		...
		$.ajax({
			...
			success : function(data) {
				alert(data);
				window.losciencecation = 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=topicsnews&topicid=' + topic;
			}
		});
		return false;
	});
	...
	// sciencecat
	$("#select-img-sciencecat").click(function() {
		var area = "image";
		var path = CFG.upload_path;
		var currentpath = CFG.upload_current;
		var type = "image";
